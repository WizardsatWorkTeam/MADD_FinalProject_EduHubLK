...
private fun initialize() {
        ...
        mRootRef.child("Classroom/$classId").addValueEventListener(object : ValueEventListener {
            override fun onCancelled(databaseError: DatabaseError) {
                Toast.makeText(this, "Error : ${databaseError.message}", Toast.LENGTH_LONG).show()
        ...
        try {
            ...
            filePath.putFile(imageUri!!).addOnSuccessListener {
                filePath.downloadUrl.addOnSuccessListener {
                    image_uri -> val uploadTask = thumb_filePath.putBytes(thumbData)
                    uploadTask.addOnSuccessListener {
                        thumb_task } -> thumb_filePath.downloadUrl.addOnSuccessListener { thumbs_uri ->
                        val glide_image: Any = when (thumbs_uri.toString()) {
